...
#   2. Create the default admin user if it does not already exist.
...
CMD set -xe; python manage.py migrate --noinput; python manage.py create_default_admin; gunicorn ler_o_brasil.wsgi:application
